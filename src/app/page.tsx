      ...
      <section className="relative min-h-screen flex items-center justify-center bg-navy overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <AnimateIn delay={0.4} duration={1}>
            <h1 className="font-serif text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-bold text-white tracking-wider mb-6">
              ATLAS
            </h1>
          </AnimateIn>
          ...
          <AnimateIn delay={0.8} duration={1}>
            <p className="font-serif text-xl md:text-3xl lg:text-4xl text-white/90 font-light mb-12">
          ...
          <AnimateIn delay={1} duration={1}>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                href="/iletisim"
                className="inline-flex items-center justify-center px-6 sm:px-8 py-4 bg-gold text-navy font-semibold rounded hover:bg-gold-light transition-colors duration-300 min-h-[48px] w-full sm:w-auto"
              >
                Ücretsiz Danışmanlık
              </Link>
              <Link
                href="/uzmanlik"
                className="inline-flex items-center justify-center px-6 sm:px-8 py-4 border border-white/30 text-white font-medium rounded hover:border-gold hover:text-gold transition-colors duration-300 min-h-[48px] w-full sm:w-auto"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {expertiseAreas.map((area, index) => (
              <AnimateIn key={area.id} delay={index * 0.1} className="h-full">
                <div className="h-full bg-navy rounded-lg p-6 sm:p-8 group hover:bg-navy-light transition-colors duration-500">
            ...
            {stats.map((stat, index) => (
              <AnimateIn key={stat.label} delay={index * 0.15}>
                <div className="text-center">
                  <span className="font-serif text-3xl md:text-5xl font-bold text-navy block mb-2">
                    {stat.value}
                  </span>
            ...
            {coreValues.map((value, index) => (
              <AnimateIn key={value.title} delay={index * 0.15}>
                <div className="text-center px-4 sm:px-6">
                  ...
                  <h3 className="font-serif text-xl sm:text-2xl font-semibold text-navy mb-4">
                    {value.title}
                  </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {caseStudies.slice(0, 3).map((cs, index) => (
              <AnimateIn key={cs.id} delay={index * 0.15}>
                <div className="bg-navy-light rounded-lg p-6 sm:p-8 border border-white/5 hover:border-gold/20 transition-colors duration-500">
                  <span className="text-xs font-medium text-gold bg-gold/10 px-3 py-1 rounded-full">
                    {cs.category}
                  </span>
                  <h3 className="font-serif text-lg sm:text-xl font-semibold text-white mt-4 mb-3">
                    {cs.title}
                  </h3>
          ...
          <AnimateIn delay={0.4}>
            <div className="text-center mt-12">
              <Link
                ...
                className="inline-flex items-center px-6 py-3 border border-gold/30 text-gold font-medium rounded hover:bg-gold hover:text-navy transition-colors duration-300 min-h-[44px]"
              >
            ...
            {testimonials.map((item, index) => (
              <AnimateIn key={item.name} delay={index * 0.15}>
                <div className="bg-white rounded-lg p-6 sm:p-8 shadow-sm">
                  <div className="text-gold text-3xl sm:text-4xl font-serif mb-4">
                    &ldquo;
                  </div>
      ...
      <section className="py-16 sm:py-24 bg-navy relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <AnimateIn delay={0.3}>
            <Link
              href="/iletisim"
              className="inline-flex items-center px-8 sm:px-10 py-4 bg-gold text-navy font-semibold text-base sm:text-lg rounded hover:bg-gold-light transition-colors duration-300 min-h-[48px]"
            >
              Ücretsiz Danışmanlık
            </Link>
          </AnimateIn>
